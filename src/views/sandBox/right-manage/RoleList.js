...
import { getRolesList, deleteRolesById, getAllRightsList } from '@/api'
...
export default function RoleList() {
        ...
        return (
          <>
            <Button danger
              shape="circle"
              icon={<DeleteOutlined />}
              onClick={() => popupDelete(item)}></Button>
              <Button
              type='primary'
              shape="circle"
              icon={<EditOutlined />}
              onClick={() => popupEdit(item)}></Button>
          </>
        )
      }
    }
  ...
  const onCheck =(checkList) => {
    setCurrentRights(checkList)
  }
  return (
    <div>
      <Table columns={columns} dataSource={rolesList} rowKey={(item)=>item.id}/>
